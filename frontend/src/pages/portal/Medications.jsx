            ...
            <div className="bg-secondary/20 p-8 rounded-3xl border border-border/50">
                <div className="flex flex-col md:flex-row items-center justify-between gap-8 mb-8">
                    <div className="flex items-center gap-4">
                        ...
                        <div>
                            <h4 className="text-xl font-bold mb-1">Daily Adherence Matrix</h4>
                            <p className="text-sm text-muted-foreground italic font-medium">Mark your doses to maintain your 94% clinical consistency score.</p>
                        </div>
                    </div>
                </div>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                    {medications.map(med => (
                        <div key={med.id} className="flex items-center justify-between p-4 rounded-xl bg-card border border-border/40 hover:border-emerald-500/30 transition-all cursor-pointer group">
                            <div className="flex items-center gap-3">
                                <div className="h-4 w-4 rounded border-2 border-emerald-500/30 group-hover:border-emerald-500 transition-colors"></div>
                                <div className="flex flex-col">
                                    <span className="text-xs font-bold leading-none mb-1">{med.name}</span>
                                    <span className="text-[10px] text-muted-foreground font-medium uppercase opacity-60">{med.frequency}</span>
                                </div>
                            </div>
                            <Badge variant="ghost" className="text-[8px] font-black uppercase tracking-tighter opacity-40">Pending</Badge>
                        </div>
                    ))}
                </div>

                <div className="mt-8 flex justify-center">
                    <Button className="h-12 px-10 rounded-xl bg-emerald-500 hover:bg-emerald-600 font-bold gap-3 shadow-xl shadow-emerald-500/10 transition-all active:scale-95">
                        Log All Morning Doses
