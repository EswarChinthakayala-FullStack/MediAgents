...
const Dashboard = () => {
    ...
    useEffect(() => {
        if (lastMessage) {
            ...
            // Map event bus message to incident
            let newIncident = null;

            if (stream === 'triage_result') {
                newIncident = {
                    patient: `Patient ${data.triage_id.slice(0, 4)}`,
                    id: data.triage_id,
                    status: data.urgency_label,
                    time: 'Just now',
                    severity: data.urgency_tier <= 1 ? 'emergency' : (data.urgency_tier <= 2 ? 'urgent' : 'selfcare')
                };
            } else if (stream === 'emergency_alert') {
                newIncident = {
            ...
            if (newIncident) {
                setRecentIncidents((prev) => [newIncident, ...prev].slice(0, 10));
            }
        }
    }, [lastMessage]);
