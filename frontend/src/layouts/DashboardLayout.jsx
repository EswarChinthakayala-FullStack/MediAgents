            ...
            return [
                {
                    label: 'Operations',
                    items: [
                        { icon: <LayoutDashboard size={14} />, label: 'Mission Control', path: '/admin/home' },
                        { icon: <Calendar size={14} />, label: 'Resource Intel', path: '/admin/scheduling' },
                        { icon: <Globe size={14} />, label: 'Population Health', path: '/admin/analytics' },
                    ]
                },
                {
                    label: 'Federated Fleet',
                    items: [
                        { icon: <Zap size={14} />, label: 'Agent Monitor', path: '/admin/agents' },
                        { icon: <Settings size={14} />, label: 'Threshold Engine', path: '/admin/alerts/config' },
                        { icon: <FileText size={14} />, label: 'Intelligence Hub', path: '/admin/reports' },
                    ]
                },
                {
                    label: 'Authority',
                    items: [
                        { icon: <Users size={14} />, label: 'IAM Directory', path: '/admin/users' },
                        { icon: <Lock size={14} />, label: 'Forensic Audit', path: '/admin/audit' },
                        { icon: <ShieldCheck size={14} />, label: 'Compliance Vault', path: '/admin/compliance' },
                        { icon: <Settings size={14} />, label: 'Kernel Settings', path: '/admin/settings' },
                    ]
                }
            ];
        }
        ...
        return [
            {
                label: 'General',
                items: [
                    { icon: <LayoutDashboard size={14} />, label: 'Live Dashboard', path: '/clinical/home' },
                    { icon: <Users size={14} />, label: 'Patient Queue', path: '/clinical/queue' },
                    { icon: <Bell size={14} />, label: 'Emergency Feed', path: '/clinical/alerts' },
            ...
            },
            {
                ...
                items: [
                    ...
                    { icon: <Activity size={14} />, label: 'Safety Monitor', path: '/clinical/safety' },
            ...
            },
            {
                ...
                items: [
                    ...
                    { icon: <FileText size={14} />, label: 'Patient Directory', path: '/clinical/patients' },
    ...
    const SidebarContent = ({ isMobile = false }) => (
        <div className="flex flex-col h-full bg-card text-muted-foreground border-r border-border ring-offset-background relative overflow-hidden">
            ...
            <div className="pt-6 px-4 flex flex-col gap-4">
                <div className="flex items-center justify-between min-h-[40px]">
                    ...
                    {!isMobile && (
                        <button
                            ...
                            className={`p-1.5 rounded hover:bg-secondary text-muted-foreground hover:text-foreground transition-all duration-200 ${isActuallyCollapsed ? 'hidden' : 'block'}`}
                            title="Close Sidebar"
                        >
                ...
                {(!isActuallyCollapsed || isMobile) && (
                    <div className="flex items-center justify-between px-3 py-2 rounded bg-secondary/40 border border-border text-[12px] text-foreground font-medium hover:bg-secondary transition-colors cursor-pointer group mb-2 animate-in slide-in-from-left-2 duration-300">
                        <div className="flex items-center gap-2.5 overflow-hidden">
                            <div className="w-4 h-4 rounded bg-primary/20 flex items-center justify-center text-primary">
                                <UserIcon size={10} />
                            </div>
                            <span className="truncate capitalize">{user?.role || 'User'} Console</span>
                        </div>
                            ...
                            {group.items.map((item) => {
                                ...
                                return (
                                    <Link
                                        key={item.label}
                                        to={item.path}
                                        className={`flex items-center gap-3 px-3 py-1.5 rounded text-[13px] font-medium transition-all duration-150 group relative ${isActive
            ...
            <div className="p-3 border-t border-border space-y-1 mt-auto bg-card z-10">
                <Link
                    ...
                    className={`flex items-center gap-3 px-3 py-2 rounded text-[13px] font-medium hover:text-foreground hover:bg-secondary/40 transition-all duration-150 group ${isActuallyCollapsed && !isMobile ? 'justify-center' : ''}`}
                >
            ...
            >
                ...
                {isActuallyCollapsed && (
                    <button
                        ...
                        className="absolute right-3 top-6 z-50 h-8 w-8 flex items-center justify-center rounded border border-border bg-card text-muted-foreground shadow-sm hover:text-primary transition-all hover:scale-110"
                        title="Open Sidebar"
                    >
                        <PanelLeft size={16} />
                    </button>
            ...
            <div className="flex flex-col flex-1 min-w-0 overflow-hidden relative">
                ...
                <header className="h-[52px] flex items-center justify-between px-4 md:px-6 border-b border-border bg-background/90 backdrop-blur-md z-30">
                    ...
                    <div className="flex items-center gap-4">
                        <div className="hidden sm:flex items-center gap-2 px-2 py-0.5 rounded border border-primary/20 bg-primary/5">
                        ...
                        <div className="flex items-center gap-3">
                            ...
                            <div className="relative hidden sm:block">
                                ...
                                <input
                                    type="text"
                                    placeholder="Search..."
                                    className="h-8 w-48 bg-secondary/50 border border-border rounded pl-8 pr-2 text-[11px] focus:w-64 focus:border-primary/50 transition-all outline-none"
                                />
